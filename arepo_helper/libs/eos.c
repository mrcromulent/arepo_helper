...
int eos_init( t_eos_table* eos_table, char* datafile, char* speciesfile ) {
	...
	if (swap == 1) {
	  printf( "swapping endianness of eos table.\n" );
	}
	...
	eos_table->datafile = (char*)malloc( strlen(datafile)+1 );
	strcpy( eos_table->datafile, datafile );
	...
	fscanf( fd, "%d", &eos_table->nspecies );
	printf( "Reading %d species from speciesfile.\n", eos_table->nspecies );
...
int eos_init_old( t_eos_table* eos_table, char* datafile, char* speciesfile ) {
    ...
    fscanf( fd, "%d", &eos_table->nspecies );
    printf( "readind %d species from speciesfile.\n", eos_table->nspecies );
...
int eos_calc_egiven( t_eos_table* eos_table, double rho, double *xnuc, double e, double *temp, double *p, double *dpdr ) {
  ...
  while(iter < EOS_MAXITER)
    {
      ...
      if (rho <= eos_table->rhoMin) {
		...
		_deedt = _ee / _temp;
  	 	...
  	 	/* do table lookup */
		eos_trilinear_e(eos_table, _temp, rho, ye, &_ee, &_deedt, &_ep);
		_ee *= 3.0 * (EOS_A1*pow(rho*ye,4./3.) + EOS_A2/3.0*temp4) / rho;
  		_deedt *= EOS_A3 * temp3 / rho;
      }

      eos_radiation(_temp, rho, &_re, &_dredt, &_rp, &_drpdt );
      eos_coulCorr(eos_table, _temp, rho, n, ni, ne, zeff, &_ce, &_dcedt, &_cp, &_dcpdr, &_dcpdt);
  ...
  if (rho <= eos_table->rhoMin) {
    ...
    /* do table lookup */
    eos_trilinear(eos_table, _temp, rho, ye, &_ee, &_deedt, &_ep, &_dpedr, &_dpedt);
...
int eos_calc_tgiven( t_eos_table* eos_table, double rho, double *xnuc, double temp, double *e, double *dedt, double *p ) {
  double *n, ni, ne, nn;	/* number densities [ particles per volume ] */
  double ye, zeff;		/* electron fraction ( electrons per baryon ), effective charge */
  double _ee, _ep;		/* energy per mass and pressure of electrons + positrons */
  ...
  if (rho > eos_table->rhoMin) {
    /* do table lookup */
    eos_trilinear_e(eos_table, temp, rho, ye, &_ee, &_deedt, &_ep);
    ...
    _ee = _deedt * temp;
	_ep = ne * BOLTZMANN * temp;
  }
  
  eos_radiation(temp, rho, &_re, &_dredt, &_rp, &_drpdt );
  eos_coulCorr(eos_table, temp, rho, n, ni, ne, zeff, &_ce, &_dcedt, &_cp, &_dcpdr, &_dcpdt);
...
void eos_coulCorr( t_eos_table* eos_table, double temp, double rho, double *n, double ni, double ne, double zeff, double *e, double *dedt, double *p, double *dpdr, double *dpdt ) {
#if 0
	/*
	 * Debye Hückel formula, just for testing, derivatives missing
	 */
	int i;
	...
	*dedt = 0;
	for ( i=0; i<eos_table->nspecies; i++ ) {
		*e += ni[i] * eos_table->nuclearcharges[i] * eos_table->nuclearcharges[i];
	}
	*e += ne;
	
	*e = - pow(ELECTRONCHARGE,3) * sqrt( M_PI / (temp * BOLTZMANN) ) * pow( (*e), 1.5 ) / rho;
	*p = (*e) * rho / 3.;
#endif
  ...
  if (temp < tfermi && temp > 0.1*tmelt && rho > 2.*rhocond) {
    ...
    *dpdr = 0;
    for(i = 0; i < eos_table->nspecies; i++) {
      ...
      cvii =
	  	0.5 * pow(gamma,1.5) * (a3 * (gamma - 1) / ((gamma + 1.) * (gamma + 1.)) - a1 * a2 / pow(gamma + a2,1.5)) +
		gamma2 * (b3 * (gamma2 - b4) / ((gamma2 + b4) * (gamma2 + b4)) - b1 * b2 / ((gamma + b2) * (gamma + b2)));


      dpdrii =
		0.5 * pow(gamma, 1.5) * (a3 * (1 + 7./9 * gamma) / ((gamma + 1.) * (gamma + 1.))
		+ a1 * (a2 + 8. / 9. * gamma) / pow(gamma + a2, 1.5))
		+ gamma2 / 9. * b3 * (5. * b4 + 3. * gamma2) / ((gamma2 + b4) * (gamma2 + b4))
