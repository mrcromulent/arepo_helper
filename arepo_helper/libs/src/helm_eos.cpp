...
static double h5(const double fi[36], double w0t, double w1t, double w2t, double w0mt, double w1mt, double w2mt, double w0d, double w1d, double w2d, double w0md, double w1md, double w2md) {
...
static double h3(const double fi[16], double w0t, double w1t, double w0mt, double w1mt, double w0d, double w1d, double w0md, double w1md) {
...
static void azbar(t_helm_eos_table* helm_eos_table, const double xnuc[], double *abar, double *zbar) {
...
// radiation
static int helm_eos_rad(double rho, double temp, double abar, double zbar, double prad[5], double erad[5], double srad[5]) {
    const double rhoi = 1.0 / rho;
    const double tempi = 1.0 / temp;
    // prad
    prad[0] = 4.0 / 3.0 * GSL_CONST_CGS_STEFAN_BOLTZMANN_CONSTANT / GSL_CONST_CGS_SPEED_OF_LIGHT * temp*temp*temp*temp;
    // dprad dd
    prad[1] = 0.0;
    // dprad dt
...
// ion section
static int helm_eos_ion(double rho, double temp, double ltemp, const struct helm_eos_cache *cache, double zbar, double pion[5], double eion[5], double sion[5]) {
...
// electron-positron
static int helm_eos_ele(t_helm_eos_table* helm_eos_table, double rho, double temp, double l10temp, double pele[5], double eele[5], double sele[5], double etaele[5], double xne[5], const struct helm_eos_cache *cache, const int only_e) {
    ...
    jat = static_cast<int>((l10temp - helm_eos_table->ltempMin) / helm_eos_table->ltempDelta);
    iat = static_cast<int>((cache->ldin - helm_eos_table->lrhoMin) / helm_eos_table->lrhoDelta);
    ...
    // various differences
    xt = std::max(0.0, (temp - helm_eos_table->temp[jat]) * dti);
    xd = std::max(0.0, (din - helm_eos_table->rho[iat]) * ddi);
    mxt = 1.0 - xt;
    mxd = 1.0 - xd;
    ...
    if (!only_e) {
        ...
        // derivative with respect to density**2 ; not needed
        ...
        pele[1]  = std::max(0.0, ye * h3(fi, si0t,   si1t,   si0mt,   si1mt, si0d,   si1d,   si0md,   si1md));
        ...
        //derivative with respect to density
        x = std::max(0.0,h3(fi, si0t,   si1t,   si0mt,   si1mt, dsi0d,  dsi1d,  dsi0md,  dsi1md));
        xne[1]   = ye * x;
...
// Coulomb corrections
static int helm_eos_coul(double rho, double temp, double abar, double zbar, double pcoul[5], double ecoul[5], double scoul[5]) {
    // fitting parameters
    const double
            a1 = -0.898004,
            b1 = 0.96786,
    ...
    plasg = (electron_charge_esu * zbar) * (electron_charge_esu * zbar) / (kt * lami);
...
int eos_init(t_helm_eos_table* helm_eos_table, const char* datafile, const char* speciesfile, const int useCoulombCorrections, const int verbose) {
    ...
    helm_eos_table->useCoulombCorrections = useCoulombCorrections;
    helm_eos_table->verbose = verbose;
    ...
    for (int j = 0; j < helm_eos_table->ntemp; j++) {
        ...
        for (int i = 0; i < helm_eos_table->nrho; i++) {
            ...
            auto scan = fscanf(file, "%lf %lf %lf %lf %lf %lf %lf %lf %lf", &helm_eos_table->f[i][j], &helm_eos_table->fd[i][j], &helm_eos_table->ft[i][j], &helm_eos_table->fdd[i][j], &helm_eos_table->ftt[i][j], &helm_eos_table->fdt[i][j], &helm_eos_table->fddt[i][j], &helm_eos_table->fdtt[i][j], &helm_eos_table->fddtt[i][j]); // NOLINT(cert-err34-c)
            if (scan != 9) {
    ...
    helm_eos_table->na = static_cast<double *>(malloc(helm_eos_table->nspecies * sizeof(double)));
    helm_eos_table->nai = static_cast<double *>(malloc(helm_eos_table->nspecies * sizeof(double)));
    helm_eos_table->nz = static_cast<double *>(malloc(helm_eos_table->nspecies * sizeof(double)));

    if (helm_eos_table->na == nullptr || helm_eos_table->nai == nullptr || helm_eos_table->nz == nullptr) {
...
int eos_calc_tgiven(t_helm_eos_table* helm_eos_table, double rho, const double xnuc[], double temp, struct eos_result *res) {
    ...
    azbar(helm_eos_table, xnuc, &cache.abar, &cache.zbar);
    helm_eos_update_cache(rho, cache.abar, cache.zbar, &cache);

    return eos_calc_tgiven_azbar(helm_eos_table, rho, &cache, temp, res, 0);
}
...
int eos_calc_tgiven_onlye(t_helm_eos_table* helm_eos_table, double rho, const double xnuc[], double temp, struct eos_result *res) {
    ...
    return eos_calc_tgiven_azbar(helm_eos_table, rho, &cache, temp, res, 1);
}

int eos_calc_tgiven_azbar(t_helm_eos_table* helm_eos_table, double rho, const struct helm_eos_cache *cache, double temp, struct eos_result *res, const int only_e) {
    ...
    if (res != nullptr) res->temp = temp;

    if (res != nullptr && !only_e) {
        double x;
        ...
        res->sound = GSL_CONST_CGS_SPEED_OF_LIGHT * sqrt(res->gamma_1 / (1.0 + (res->e.v + (GSL_CONST_CGS_SPEED_OF_LIGHT*GSL_CONST_CGS_SPEED_OF_LIGHT)) * rho / res->p.v));
...
//    _Bool have_to_free = false;
    ...
    if (tempguess == nullptr || *tempguess <= 0.0) {
    ...
    if (res == nullptr) {
        res = static_cast<eos_result *>(malloc(sizeof(struct eos_result)));
        have_to_free = true;
    }
    ...
    for (iter = 0; iter < HELM_EOS_MAXITER; iter++) {
        ...
        _temp = std::max(std::min(_temp, helm_eos_table->tempMax), helm_eos_table->tempMin);

    }

    if (iter >= HELM_EOS_MAXITER) {
        if(helm_eos_table->verbose)
            fprintf(stderr, "Newton-Raphson in function `%s' did not converge, _temp=%g, rho=%g.\n", __func__, _temp, rho);
        return -1;
    }
    ...
    if (eos_calc_tgiven_azbar(helm_eos_table, rho, &cache, _temp, res, 0) != 0) return -1;

    /* FIXME: dpdr should be checked again */
    /* *dpdr = res->p.drho + _temp * (res->p.dtemp / rho)*(res->p.dtemp / rho) / res->e.dtemp;
       assert(*dpdr > 0); */
...
//    _tempold = 0.0;
    for (iter = 0; iter < HELM_EOS_MAXITER; iter++) {
        ...
        // not converged; compute the next step
        _tempold = _temp;
        _temp = _temp - (res->p.v - p) / res->p.dtemp;
    ...
    if (iter >= HELM_EOS_MAXITER) {
        if(helm_eos_table->verbose)
            fprintf(stderr, "Newton-Raphson in function `%s' did not converge (temp=%g, rho=%g).\n", __func__, _temp, rho);
        return -1;
    }
...
int eos_calc_egiven_y(t_helm_eos_table* helm_eos_table, double rho, const double y[], double e, double *tempguess, struct eos_result *res) {
    ...
    xnuc = static_cast<double *>(malloc(nspecies * sizeof(double)));
...
//    _Bool have_to_free = false;
    ...
    azbar(helm_eos_table, xnuc, &abar, &zbar);
...
//    _rhoold = 0.0;
    for (iter = 0; iter < HELM_EOS_MAXITER; iter++) {
        ...
        _rho = std::max(std::min(_rho, helm_eos_table->rhoMax), helm_eos_table->rhoMin);
    }
...
int helm_eos_update_cache(double rho, double abar, double zbar, struct helm_eos_cache *cache) {
    ...
    cache->lswot15 = 1.5 * log((
            2.0 * M_PI * GSL_CONST_CGS_UNIFIED_ATOMIC_MASS * GSL_CONST_CGS_BOLTZMANN) /
