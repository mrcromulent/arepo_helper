...
PyObject* pyOpalEos_getrhovalues(pyOpalEos* self, PyObject* args) {
    int i;
    auto rhos = (PyListObject *)PyList_New(NRHO);
    for (i = 0; i < NRHO; i++)
...
PyObject* pyOpalEos_gettvalues(pyOpalEos* self, PyObject* args) {
    int i;
    auto ts = (PyListObject *)PyList_New(NT);
    for (i = 0; i < NT; i++)
...
PyObject* pyOpalEos_gettlim(pyOpalEos* self, PyObject* args) {
    int i;
    auto mints = (PyListObject *)PyList_New(NRHO);
    for (i = 0; i < NRHO; i++)
...
int pyConvertOpalEos(PyObject* object, struct opal_eos_table** opal_eos_table) {
    if (strcmp(object->ob_type->tp_name, "pyopaleos") != 0) {
        PyErr_BadArgument();
        return 0;
    }
...
PyObject* loadopaleos(PyObject *self, PyObject *args) {
    char *datafile;
    struct opal_eos_table* opal_eos_table;
...
static PyMethodDef pyopaleosMethods[] = {
        { "loadopal_eos", loadopaleos, METH_VARARGS,
