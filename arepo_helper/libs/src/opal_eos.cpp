 ...
 *
 * implementation of the OPAL equation of state in C using
 * interpolation of the freely available data
 *
 * @author Sebastian Ohlmann <sohlmann@astro.uni-wuerzburg.de>
 * @date 09/2013
 *
 * update: implemented iteration on pressure (pgiven functions)
 * @author Sebastian Ohlmann <sohlmann@astro.uni-wuerzburg.de>
 * @date 07/2015
 */

/* If zlib is used, -lz is required for linking */
#undef USE_ZLIB
...
static inline double interpolate_linear(double x, const double *xs, double *ys);
static inline double interpolate_linear_slope(double slope, double *ys);
static inline double interpolate_quadratic(double x, const double *xs, const double *ys);
static inline double interpolate_quadratic_coeff(double A, double B, double C, const double *ys);
static inline void binary_search(double x, const double *list, int len, int points, int *low, int *m);
static inline int linear_search_asc(double x, const double *list, int len);
...
void compute_quadratic_coeff(double x, const double *xs, double *A, double *B, double *C);
...
{
    /* allocate table */
    auto opal_table = (struct opal_eos_table *) malloc(sizeof(struct opal_eos_table));

    if(opal_table == nullptr)
    {
    ...
    /* allocate cache */
    opal_table->cache = (struct opal_eos_cache *)
            malloc(sizeof(struct opal_eos_cache));

    if(opal_table->cache == nullptr)
    {
    ...
    for(ix = 0; ix < NX; ix++)
    {
        ...
        if(fgets(line, 200, datafile) == nullptr)
        {
        ...
        for(irho = 0; irho < NRHO; irho++)
        {
            ...
            n = sscanf(line, "%d %d %lf %lf density(g/cc)= %lf", &jrho,
                       &opal_table->numt[irho], &dum, &dum, &opal_table->rho[irho]);
            /* consistency checks */
            if(n < 5)
            {
            ...
            for(it = 0; it < opal_table->numt[irho]; it++)
            {
                ...
                n = sscanf(line, "%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf%lf",
                           ...
                           &opal_table->chir[it][irho][ix], &opal_table->chit[it][irho][ix],
            ...
            int mode, struct opal_eos_result *result)
{
    ...
    /* can be commented in to restrict EOS to OPAL range */
    /*
    if (do_opal == -1)
      {
        rho = dmax(opal_table->minrho, dmin(opal_table->maxrho, rho));
        double mint = dmax(opal_table->mint[opal_table->cache->mrho],
                           opal_table->mint[opal_table->cache->mrho + 1]);
        t6 = dmax(mint, dmin(opal_table->maxt, t6));
        t = t6 * 1e6;
      }
    do_opal = 1;
    */
    ...
    (void)err;
    p0 = 0;
    gamma1_0 = gamma2_0 = gamma3_0 = 0;
    gamma1_tot = gamma2_tot = gamma3_tot = 0;
                     ...
                     int mode, struct opal_eos_result *result)
{
    ...
    double mu;
    /* introduces discontinuity -> treat everything as ionized?
...
//          return -1;
    ...
    if (num_out > 5 || i >= OPAL_NEWTON_MAX)
    {
        ...
        for (i = 0; i < OPAL_NEWTON_MAX; i++)
        {
            ...
            /* converged to desired accuracy? */
            if (fabs(de) <= OPAL_NEWTON_ACCURACY * e) break;

            /* bisection stopped at border? */
            if (fabs(told - t) <= OPAL_NEWTON_ACCURACY * t) num_out++;
                         ...
                         double p, double *tguess, int mode, struct opal_eos_result *result)
{
    ...
    */
    ...
    for (i = 0; i < OPAL_NEWTON_MAX; i++)
    {
        ...
        /* converged to desired accuracy? */
        if (fabs(dp) <= OPAL_NEWTON_ACCURACY * p) break;
...
/**
 * @brief driver routine for retrieving all eos values given the pressure and temperature
 *
 * @param opal_table the eos table struct
 * @param x hydrogen mass fraction
 * @param p pressure
 ...
 * @return <0 if error occured, 0 otherwise
 */
int opaleos_all_ptgiven_local(struct opal_eos_table *opal_table, double x, double p,
                              double t, struct opal_eos_result *result)
{
    return opaleos_ptgiven_local(opal_table, x, p, t, EOS_ALL, result);
}
 ...
 * @return <0 if error occured, 0 otherwise
 */
int opaleos_ptgiven_local(struct opal_eos_table *opal_table, double x, double p,
                          double t, int mode, struct opal_eos_result *result)
{
    double rho = 0;  /* temperature */
    double dp;     /* change in pressure */
    //double egas;
    int i, err;
    double rho1, rho2, p1, p2, rhotry;

    /* try value for rho */
    double A = mean_molecular_weight(x, opal_table->z) / GAS_CONSTANT;
    double B = 4.0 / 3.0 * SIGMAC;
    rhotry = A / t * (p - B * t*t*t*t);

    /* check bounds, if out of bounds move inside */
    /* Disable!
    if (rhotry < opal_table->minrho) rhotry = opal_table->minrho;
    if (rhotry > opal_table->maxrho) rhotry = opal_table->maxrho;
    rho = rhotry;
    */

#ifdef OPAL_DEBUG
    fprintf(stderr, "Called with X=%e, p=%e, t=%e; rhotry=%e\n", x, p, t, rhotry);
#endif


    //update_cache(opal_table, x, rhotry, t*1e-6);
    /* get pressure at boundaries */
    //rho1 = opal_table->minrho * 1.0001;
    rho1 = 1e-20;
    err = opaleos(opal_table, x, rho1, t, EOS_PRESSURE | EOS_RADIATION, result);
    p1 = result->p;
#ifdef OPAL_DEBUG
    fprintf(stderr, "X=%e, rho=%e, p=%e, t=%e\n", x, rho1, result->p, t);
#endif
    //rho2 = opal_table->maxrho * 0.9999;
    rho2 = opal_table->maxrho * 10.0;
    /* search max rho for which T > minT */
    /*
    for (i = NRHO - 1; i > 0; i--)
        if (t > opal_table->mint[i]*1e6)
          {
            rho2 = opal_table->rho[i-1];
            break;
          }
          */
    err = opaleos(opal_table, x, rho2, t, EOS_PRESSURE | EOS_RADIATION, result);
    p2 = result->p;
#ifdef OPAL_DEBUG
    fprintf(stderr, "X=%e, rho=%e, p=%e, t=%e\n\n\n", x, rho2, result->p, t);
#endif
    ...
    {
        rho2 = rho;
        rhotry = fmax(rho/3., rho1);
    }
    else
    {
        rho1 = rho;
        rhotry = fmin(rho*3., rho2);
    }
    err = opaleos(opal_table, x, rhotry, t, EOS_PRESSURE | EOS_RADIATION, result);
#ifdef OPAL_DEBUG
    fprintf(stderr, "X=%e, rho=%e, p=%e, t=%e\n\n\n", x, rhotry, result->p, t);
#endif
    dp = (result->p - p);
...
#ifdef OPAL_DEBUG
        fprintf(stderr, "Iteration %d for X=%e, p=%e, t=%e  => rho=%e, drho=%e, p_n=%e, dp=%e, dp/p=%e\n", i, x, p, t, rho, rho -rhoold, result->p, dp, dp/p);
#endif
    }
 ...
 * @return <0 if error occured, 0 otherwise
 */
int opaleos_all_ptgiven_global(double x, double p, double t,
    struct opal_eos_result *result)
{
  return opaleos_all_ptgiven_local(opal_table, x, p, t, result);
}

int opaleos_ptgiven_global(double x, double p, double t,
    int mode, struct opal_eos_result *result)
{
  return opaleos_ptgiven_local(opal_table, x, p, t, mode, result);
}
#endif
 ...
 * @result negative if error occured, otherwise zero
 */
int get_eostable_value(struct opal_eos_table *opal_table, eostable var,
                       double x, double rho, double t, double *result)
{
    int i, j;
  ...
  topright  = !( (rhoposition == LEFT) || (opal_table->cache->mt == opal_table->cache->it + 2)
      || (imaxt2 < opal_table->cache->it + 2) );
  bottomright = !( (rhoposition == LEFT) || (opal_table->cache->mt == opal_table->cache->it)
      || (imaxt2 < opal_table->cache->it + 3) );
  ...
  if (opal_table->cache->mx == 0)
    {
      for (i = 0; i < 4; i++)
        {
          if (i == 0 && !top) continue;
          if (i == 4 && !bottom) continue;
          for (j = 0; j < 4; j++)
            {
              if (j == 0)
                {
                  if (!left) continue;
                  if (i == 0 && !topleft) continue;
                  if (i == 4 && !bottomleft) continue;
                }
              if (j == 4)
                {
                  if (!right) continue;
                  if (i == 0 && !topright) continue;
                  if (i == 4 && !bottomright) continue;
                }
              varrt[i][j] = interpolate_quadratic_coeff(opal_table->cache->Ax1,
                  opal_table->cache->Bx1, opal_table->cache->Cx1,
                  var[opal_table->cache->it + i][opal_table->cache->irho + j]
                  + opal_table->cache->ix);
            }
        }
    }
          ...
          for (j = 0; j < 4; j++)
            {
              ...
              varrt[i][j] = interpolate_quadratic_coeff(opal_table->cache->Ax2,
                  opal_table->cache->Bx2, opal_table->cache->Cx2,
                  var[opal_table->cache->it + i][opal_table->cache->irho + j]
                  + opal_table->cache->ix + 1);
            }
        }
    }
  /* in the middle */
          ...
          for (j = 0; j < 4; j++)
            {
              ...
              varrt2[i][j] = interpolate_quadratic_coeff(opal_table->cache->Ax2,
                  ...
                  var[opal_table->cache->it + i][opal_table->cache->irho + j]
                  + opal_table->cache->ix + 1);
              varrt[i][j] = opal_table->cache->slopex * varrt1[i][j] +
                              (1.0 - opal_table->cache->slopex) * varrt2[i][j];
            }
        }
    }
      ...
      for (i = 0; i < 4; i++)
        {
          if (i == 0 && !topleft) continue;
          if (i == 4 && !bottomleft) continue;
          vart1[i] = interpolate_quadratic_coeff(opal_table->cache->Arho1,
              opal_table->cache->Brho1, opal_table->cache->Crho1, varrt[i]);
        }
    }
  /* right border */
      ...
      for (i = 0; i < 4; i++)
        {
          if (i == 0 && !topright) continue;
          if (i == 4 && !bottomright) continue;
          vart2[i] = interpolate_quadratic_coeff(opal_table->cache->Arho2,
              opal_table->cache->Brho2, opal_table->cache->Crho2, varrt[i] + 1);
        }
    }
  /* in the middle */
  else
  ...
  else if (topleft)  var12 = var1;
  else if (bottomleft) var12 = var2;

  if (topright && bottomright)
    var34 = opal_table->cache->slopet * var3 + (1.0 - opal_table->cache->slopet) * var4;
  else if (topright)  var34 = var3;
  else if (bottomright) var34 = var4;

  /* overlap in density */
  if (left && right)
    *result = opal_table->cache->sloperho * var12 + (1.0 - opal_table->cache->sloperho) * var34;
  else if (left)    *result = var12;
  else if (right)   *result = var34;
 ...
 */
double interpolate_linear(double x, const double *xs, double *ys)
{
 ...
 * @return interpolated value
 */
double interpolate_quadratic(double x, const double *xs, const double *ys)
{
 ...
 * @return interpolated value
 */
double interpolate_quadratic_coeff(double A, double B, double C, const double *ys)
{
    return A * ys[0] + B * ys[1] + C * ys[2];
}
 ...
 */
void binary_search(double x, const double *list, int len, int points, int *low, int *m)
{
 ...
 */
int linear_search_asc(double x, const double *list, int len)
{
    int ilow = 0;
...
int update_cache(struct opal_eos_table *opal_table, double x, double rho, double t)
{
    ...
    if (   (fabs(opal_table->cache->t - t) > 1e-15)
           || (fabs(opal_table->cache->mint - mint) > 1e-15))
    {
...
for x=%e, rho=%e.\n", t * 1e6,
        ...
        binary_search(t, opal_table->t, minnum, num_points,
    ...
    /* set slopes */
    if (fabs(opal_table->cache->x - x) > 1e-15)
        opal_table->cache->slopex = (x - opal_table->x[opal_table->cache->ix]) /
                                    (opal_table->x[opal_table->cache->ix + 1] - opal_table->x[opal_table->cache->ix]);
    if (fabs(opal_table->cache->rho - rho) > 1e-15)
    ...
    if (   (fabs(opal_table->cache->t - t) > 1e-15)
           || (fabs(opal_table->cache->mint - mint) > 1e-15))
        opal_table->cache->slopet = (t - opal_table->t[opal_table->cache->it]) /
                                    (opal_table->t[opal_table->cache->it + 1] - opal_table->t[opal_table->cache->it]);
#elif INTERPOLATION_METHOD == IP_QUADRATIC
    /* update coefficients only if values change */
  if (fabs(opal_table->cache->x - x) > 1e-15)
    {
      /* set A, B and C coefficients */
      compute_quadratic_coeff(x, opal_table->x + opal_table->cache->ix,
          &opal_table->cache->Ax1, &opal_table->cache->Bx1, &opal_table->cache->Cx1);
      compute_quadratic_coeff(x, opal_table->x + opal_table->cache->ix + 1,
          &opal_table->cache->Ax2, &opal_table->cache->Bx2, &opal_table->cache->Cx2);
  ...
  if (fabs(opal_table->cache->rho - rho) > 1e-15)
    {
      /* set A, B and C coefficients */
      compute_quadratic_coeff(rho, opal_table->rho + opal_table->cache->irho,
          &opal_table->cache->Arho1, &opal_table->cache->Brho1, &opal_table->cache->Crho1);
      compute_quadratic_coeff(rho, opal_table->rho + opal_table->cache->irho + 1,
          &opal_table->cache->Arho2, &opal_table->cache->Brho2, &opal_table->cache->Crho2);
      /* set slopes for overlapping quadratics */
  ...
  if (   (fabs(opal_table->cache->t - t) > 1e-15)
    ...
    {
      /* set A, B and C coefficients */
      compute_quadratic_coeff(t, opal_table->t + opal_table->cache->it,
          &opal_table->cache->At1, &opal_table->cache->Bt1, &opal_table->cache->Ct1);
      compute_quadratic_coeff(t, opal_table->t + opal_table->cache->it + 1,
          &opal_table->cache->At2, &opal_table->cache->Bt2, &opal_table->cache->Ct2);
      /* set slopes for overlapping quadratics */
