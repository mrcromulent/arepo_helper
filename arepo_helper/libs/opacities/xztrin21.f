      subroutine opacity(xh, rho, t, kappa)
      real, intent(in) :: xh, rho, t
      ...
      r = rho/t6**3
      ! check boundaries
      if (log10(t) < 3.75 .or. log10(t) > 8.7 .or. log10(r) < -8 .or.
     &    log10(r) > 1) then
c      print*,"Error: logT (3.75 to 8.7) or logR (-8 to 1)out of bounds!"
c      print*,"logT: ", log10(t), "logR: ", log10(r)
      kappa = 0
      return
      end if


      ! get opacity for solar metallicity (Z=0.02, mzin=8)
      call opac(0, 8, xh, t6, r)

      kappa = 10**opact
      end subroutine

      subroutine init(pathname)
...
c                  with a return 
      stop
c   64 write(*,'(" X not equal to zero: To run this case it
c     .is necessary"/ "to recompile with parameter (mx=1)")')
c      stop
...
c
      close (2)
c..... read  tables
       open(2, FILE=trim(path)//'GN93hz', iostat=ios)
c       print*,'Open file ', trim(path)//'GN93hz', ', iostat: ', ios
c
c      read header
      do i=1,240
      read (2,'(a)') dumarra(i)
...
C
C
c 1000 FORMAT('  SMOOTHED OPAL DATA')
c 1100 FORMAT('  OPAL DATA, (SMOOTHED-ORIGINAL)')
c 2000 FORMAT(100A1)
c 2222 FORMAT(F8.3,20F7.3)
c 6000 FORMAT(/' FIRST T6=',1P,E10.3,', SHOULD BE 0.006')
c 6003 FORMAT(/' !!! OUT-OF-RANGE !!!'/' FLT=',1P,E10.3,', FLRHO=',E10.3,
c     + ', FLR=',E10.3)
C
      END
C
