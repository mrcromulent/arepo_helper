...
// radiation
static int __attribute__((unused)) helm_eos_rad(t_helm_eos_table* helm_eos_table, double rho, double temp, double abar, double zbar, double prad[5], double erad[5], double srad[5]) {
...
// ion section
static int __attribute__((unused)) helm_eos_ion(t_helm_eos_table* helm_eos_table, double rho, double temp, double ltemp, const struct helm_eos_cache *cache, double abar, double zbar, double pion[5], double eion[5], double sion[5]) {
...
// Coulomb corrections
static int __attribute__((unused)) helm_eos_coul(t_helm_eos_table* helm_eos_table, double rho, double temp, double abar, double zbar, double pcoul[5], double ecoul[5], double scoul[5]) {
  // fitting parameters
  const double
    a1 = -0.898004,
    b1 = 0.96786,
...
int eos_calc_tgiven(t_helm_eos_table* helm_eos_table, double rho, const double xnuc[], double temp, struct eos_result *res) {
  ...
  helm_eos_update_cache(helm_eos_table, rho, cache.abar, cache.zbar, &cache);
...
int eos_calc_tgiven_azbar(t_helm_eos_table* helm_eos_table, double rho, const struct helm_eos_cache *cache, double temp, struct eos_result *res, const int only_e) {
  ...
  if (helm_eos_ion(helm_eos_table, rho, temp, ltemp, cache, abar, zbar, pion, eion, sion) != 0) return -1;
#ifndef EOS_IDEAL
  if (helm_eos_rad(helm_eos_table, rho, temp, abar, zbar, prad, erad, srad) != 0) return -1;
...
//#ifdef EOS_COULOMB_CORRECTIONS
  if(helm_eos_table->useCoulombCorrections)
    {
      if (helm_eos_coul(helm_eos_table, rho, temp, abar, zbar, pcoul, ecoul, scoul) != 0) return -1;
