...
def gamma_c(res: dict[str]) -> float:
    pres = res[n.PRESSURE]
    temp = res[n.TEMPERATURE]
    dedT = res["dedT"]
    dpdT = res["dpdT"]
    dpdRho = res["dpdRho"]
    rho = res[n.DENSITY]
    return (dpdRho + temp / dedT * (dpdT / rho) ** 2) * rho / pres
...
class ArepoSimulation:
    ...
    def check_requirement_met(self,
        ...
        if op == "EQ":
            return bool(req_actual_value == req_value)
        elif op == "NEQ":
            return bool(req_actual_value != req_value)
        elif op == "MAX":
            return bool(req_actual_value <= req_value)
    ...
    def validate_input_file(self,
        ...
        for i in range(n_particles):
            ...
            if self.config.get("EOS_DEGENERATE"):
                ...
                res = eos.egivenfull(density[i], xnuc[i], intern_ener[i], -1)

                if res[n.TEMPERATURE] <= 0.0:
                    raise ValueError(f"{res[n.TEMPERATURE]=}")
                else:
                    if res[n.TEMPERATURE] > max_temp:
                        res[n.TEMPERATURE] = max_temp
                        raise ValueError()

                    elif res[n.TEMPERATURE] < min_temp:
                        res[n.TEMPERATURE] = min_temp
                        raise ValueError()

                    # res = eos.tgivenfull(density[i], xnuc[i], temp_calculated)

                if res[n.TEMPERATURE] < min_temp or res[n.TEMPERATURE] > max_temp:
                    egy_injection = res[n.INTERNALENERGY] - intern_ener[i]
                    print(f"{res[n.INTERNALENERGY]=}, {egy_injection=}")
                    raise ValueError()
