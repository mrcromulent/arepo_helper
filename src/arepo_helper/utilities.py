from matplotlib.colors import Normalize, SymLogNorm
from matplotlib.cm import ScalarMappable
from typing import List
from .names import n
import os


common_snapbases = ["snapshot", "snap", "sh"]
valid_orientations = [["x", "y"], ["x", "z"], ["y", "z"]]
default_cmap = "inferno"
plot_quantities = [n.DENSITY, n.PRESSURE, n.MASSES, n.INTERNALENERGY, n.NUCLEARCOMPOSITION, n.TEMPERATURE]


arepo_git_versions = {
    "ivo_2016":
        {
            "commit_id": "3d675e62e42340566d7626178284652f3e482305",
            "url": None
        },
    "dissipative":
        {
            "commit_id": "97863140b478c1319cec0fd2f29258d6d36b5927",
            "url": None
            # "url": "https://github.com/boywert/dissipative.git"
        },
    "public_2021":
        {
            "commit_id": "2763ddc00994fd524504c3b43d836ec97d7f60c7",
            # "url": "https://gitlab.mpcdf.mpg.de/g-fmarinac/arepo.git"
            "url": None
        },
}


ruediger_conversion_table = {n.COORDINATES: "pos",
                             n.VELOCITIES: "vel",
                             n.NUCLEARCOMPOSITION: "xnuc",
                             n.INTERNALENERGY: "u",
                             n.PARTICLEIDS: "id",
                             n.MASSES: "mass",
                             n.VOLUME: "vol",
                             n.PRESSURE: "pres",
                             n.PASSIVESCALARS: "pass",
                             n.DENSITY: "rho",
                             n.TEMPERATURE: "temp",
                             n.POTENTIAL: "pot"}


part_fields = {n.CENTEROFMASS: {"Dim": None, "Units": "cm", "cmap": default_cmap, "abs_bounds": None},
               n.COORDINATES: {"Dim": 3, "Units": "cm", "cmap": default_cmap, "abs_bounds": None},
               n.DENSITY: {"Dim": None, "Units": "g / cm^3", "cmap": default_cmap, "abs_bounds": None},
               n.DENSITYGRADIENT: {"Dim": 3, "Units": "g / cm^3 / cm", "cmap": default_cmap, "abs_bounds": None},
               n.INTERNALENERGY: {"Dim": None, "Units": "erg", "cmap": default_cmap, "abs_bounds": None},
               n.MAGNETICFIELD: {"Dim": 3, "Units": "10^4 G", "cmap": default_cmap, "abs_bounds": None},
               n.MAGNETICFIELDDIVERGENCE: {"Dim": None, "Units": "10^4 G / cm^3", "cmap": default_cmap,
                                           "abs_bounds": None},
               n.MASSES: {"Dim": None, "Units": "g", "cmap": default_cmap, "abs_bounds": None},
               n.NUCLEARCOMPOSITION: {"Dim": None, "Units": None, "cmap": default_cmap, "abs_bounds": None},
               n.NUCLEARENERGYGENERATIONRATE: {"Dim": None, "Units": "erg", "cmap": default_cmap, "abs_bounds": None},
               n.PARTICLEIDS: {"Dim": None, "Units": None, "cmap": default_cmap, "abs_bounds": None},
               n.PASSIVESCALARS: {"Dim": 4, "Units": None, "cmap": default_cmap, "abs_bounds": [0.001, 1]},
               n.POTENTIAL: {"Dim": None, "Units": "erg/g", "cmap": default_cmap, "abs_bounds": None},
               n.PRESSURE: {"Dim": None, "Units": "dynes", "cmap": default_cmap, "abs_bounds": None},
               n.PRESSUREGRADIENT: {"Dim": None, "Units": "dynes/cm", "cmap": default_cmap, "abs_bounds": None},
               n.TEMPERATURE: {"Dim": None, "Units": "K", "cmap": default_cmap, "abs_bounds": None},
               n.VELOCITIES: {"Dim": 3, "Units": "cm/s", "cmap": default_cmap, "abs_bounds": None},
               n.VELOCITYDIVERGENCE: {"Dim": None, "Units": "1/s/cm^2", "cmap": default_cmap, "abs_bounds": None}
               }


class Coordinates:
    """Class to work with Coordinates"""

    x = 0
    y = 1
    z = 2

    @classmethod
    def coordinates(cls, orientation):
        """Returns coordinates in a tuple

        :param orientation: orientation list
        :type orientation: list
        :return: Number representing orientation
        :rtype: (int, int, int)
        """
        xstr, ystr = orientation
        xval = getattr(cls, xstr)
        yval = getattr(cls, ystr)
        zset = {cls.x, cls.y, cls.z} - {xval, yval}

        return xval, yval, list(zset)[0]


class SuppressStdout:
    """
    A context manager for doing a "deep suppression" of stdout and stderr in
    Python, i.e. will suppress all print, even if the print originates in a
    compiled C/Fortran sub-function.
       This will not suppress raised exceptions, since exceptions are printed
    to stderr just before a script exits, and after the context manager has
    exited (at least, I think that is why it lets exceptions through).
    """

    def __init__(self):
        # Open a pair of null files
        self.null_fds = [os.open(os.devnull, os.O_RDWR) for _ in range(2)]
        # Save the actual stdout (1) and stderr (2) file descriptors.
        self.save_fds = [os.dup(1), os.dup(2)]

    def __enter__(self):
        # Assign the null pointers to stdout and stderr.
        os.dup2(self.null_fds[0], 1)
        os.dup2(self.null_fds[1], 2)

    def __exit__(self, *_):
        # Re-assign the real stdout/stderr back to (1) and (2)
        os.dup2(self.save_fds[0], 1)
        os.dup2(self.save_fds[1], 2)
        # Close all file descriptors
        for fd in self.null_fds + self.save_fds:
            os.close(fd)


def get_cmap(quantity: str,
             bounds: List[float],
             log_cmap: bool = False) -> (str, SymLogNorm, ScalarMappable):
    """Gets the appropriate colormap

    :param quantity: One of the particle fields from the enum
    :type quantity: str
    :param bounds: A list containing the minimum value in the 0th position and the maximum in the first
    :type bounds: list
    :param log_cmap: Bool to indicate whether a logarithmic colormap is desired
    :type log_cmap: bool
    :return: Tuple of cmap, norm, scmp
    :rtype: (str, SymLogNorm, ScalarMappable)
    """

    assert quantity in plot_quantities, f"Cmap requested for unknown quantity {quantity}"

    cmap = part_fields[quantity]["cmap"]

    if part_fields[quantity]["abs_bounds"] is not None:
        bounds = part_fields[quantity]["abs_bounds"]

    if log_cmap:
        norm = SymLogNorm(bounds[0], vmin=bounds[0], vmax=bounds[1], base=10)
    else:
        norm = Normalize(bounds[0], bounds[1])

    scmp = ScalarMappable(norm=norm, cmap=cmap)

    return cmap, norm, scmp


def dummy():
    """Dummy function that does nothing"""
    pass


def convert_to_ruediger_dict(d: dict[str]) -> dict[str]:
    """Converts to Ruediger-style dictionary

    :param d: dict with Ruediger-style names
    :type d: dict[str]

    :return: dict with my style of names
    :rtype: dict[str]
    """

    new_d = dict()

    for key in d:
        if key in ruediger_conversion_table:
            new_key = ruediger_conversion_table[key]
            new_d[new_key] = d[key]
        else:
            print(f"Unconverted key: {key}")

    return new_d


def convert_from_ruediger_dict(d: dict) -> dict:
    """Converts from Ruediger-style dictionary

    :param d: dict with my style of names
    :type d: dict

    :return: dict with Ruediger-style names
    :rtype: dict
    """

    inv_map = {v: k for k, v in ruediger_conversion_table.items()}

    new_d = dict()

    for key in d:
        if key in inv_map:
            new_key = inv_map[key]
            new_d[new_key] = d[key]
        else:
            print(f"Unconverted key: {key}")

    return new_d


def sci(num: float) -> str:
    """Converts to scientific notation

    :param num: number
    :type num: float
    :return: number in scientific notation
    :rtype: str
    """
    return "{:e}".format(num)
