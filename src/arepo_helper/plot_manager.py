...
def quick_nuclear_compositions(s: ArepoSnapshot,
                               ...
                               explicit_options: Optional[dict] = None) -> Any:
    ...
    plt.ylabel("Nuclear Composition")
    plt.legend()
    plt.show()
...
def quick_nuclear_pcolors(s: ArepoSnapshot,
                          asl: ArepoSpeciesList,
                          explicit_options: dict = None) -> list[PColorPlot]:
...
def make_radial_time_series(ar: ArepoRun,
                            ...
                            a: Optional[list[float]] = None,
                            b: Optional[list[float]] = None,
                            explicit_options: Optional[dict] = None) -> None:
    ...
    for i, s in enumerate(ar.snapshots):
        ...
        t = float(s.get_from_h5(ArepoHeader.TIME))
...
def compute_plot_options_array(ar: ArepoRun,
                               ...
                               explicit_options: Optional[dict] = None,
                               ts: Optional[list[int]] = None) -> np.ndarray:
